...
# --- RED DE SEGURIDAD FINAL (CRÍTICO PARA LEDOIT-WOLF) ---
    ...
    lw.fit(Y)
    S = lw.covariance_
    # Include a Tikhonov regularization to force the method to work
    epsilon = 1e-6
    ...
    # define constraintsts
    u_ = np.ones(S.shape[0]) # S is a square matrix of (N-1)x(N-1)
...
pickle.dump(df_concat_spps, open(r'Data\Fused_pcp\TCH_DFrames\LWShrink\caso_10000_TCH_reg_LW_SPP_results_complete.p', "wb" ))
pickle.dump(df_fusion_complete, open(r'Data\Fused_pcp\TCH_DFrames\LWShrink\caso_10000_TCH_reg_LW_DATAFRAME_complete.p', "wb" ))


 #%%TESTING THE LW SHRINKAGE (SOLO PARA PRUEBAS)
# Array M x N with the differences between datasets
# M: Number of timesteps (rows)
# N: Number of datasets. For TCH -> N = 3
# N-1 represents the last product since Python take index from 0
...
#%% Identifiacion de NaN
filas_totalmente_nulas = pcp_array_3.isna().all(axis=1)

cantidad = filas_totalmente_nulas.sum()
print(f"Cantidad de filas completamente vacías: {cantidad}")

# B. ¿Cuáles son? (Obtener los índices/fechas)
indices_vacios = pcp_array_3.index[filas_totalmente_nulas]

print("Los índices (tiempos) de las filas vacías son:")
print(indices_vacios)

# Si quieres ver el sub-dataframe solo con esas filas para confirmar:
df_vacios = pcp_array_1[filas_totalmente_nulas]
# print(df_vacios)